...
#include "stuck_scene_capture_panel.hpp"
...
StuckSceneCapturePanel::StuckSceneCapturePanel(QWidget * parent) : rviz_common::Panel(parent)
{
  ...
  connect(timer, &QTimer::timeout, this, &StuckSceneCapturePanel::update);
  timer->start(1000);
  capture_timer_ = new QTimer(this);
  connect(capture_timer_, &QTimer::timeout, this, &StuckSceneCapturePanel::onTimer);
  state_ = State::WAITING_FOR_CAPTURE;
}

void StuckSceneCapturePanel::onCaptureTrigger(
...
void StuckSceneCapturePanel::onRateChanged()
{
}

void StuckSceneCapturePanel::onClickScreenCapture()
{
...
void StuckSceneCapturePanel::onClickVideoCapture()
{
...
void StuckSceneCapturePanel::onTimer()
{
  if (!main_window_) return;
...
void StuckSceneCapturePanel::update()
{
  setFormatDate(ros_time_label_, rclcpp::Clock().now().seconds());
}

void StuckSceneCapturePanel::save(rviz_common::Config config) const
{
  Panel::save(config);
}

void StuckSceneCapturePanel::load(const rviz_common::Config & config)
{
  Panel::load(config);
}

StuckSceneCapturePanel::~StuckSceneCapturePanel() = default;

#include <pluginlib/class_list_macros.hpp>
PLUGINLIB_EXPORT_CLASS(StuckSceneCapturePanel, rviz_common::Panel)
